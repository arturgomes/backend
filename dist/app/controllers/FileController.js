...
class FileController {
  async index(req,res){
    ...
    const files = await _File2.default.findAll({where:{ retail_id} })
  ...
  async store(req, res) {
    ...
    const file = await _File2.default.findOne({ where: { retail_id} });
    if(file){
    ...
    const post = await _File2.default.create({
  ...
  async delete(req,res){
    ...
    const file = await _File2.default.findOne({ where: { id} });
    var s3 = new _awssdk2.default.S3();
    var params = {
      Bucket: 'cfeedbucket',
      Key: file.key
