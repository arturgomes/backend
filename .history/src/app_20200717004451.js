  ...
  middlewares() {
    ...
    passport.use(
      new GoogleStrategy(
        ...
        async (accessToken, refreshToken, profile, done) => {
          console.log(profile);
          const newUser = {name: profile.displayName,
          ...
          // done(null,profile);
          await User.findOne({ where: { email: profile._json.email } })
            .then(user => {
              if (user)
                done(null, user)
               User.create({
                ...
                .catch(error => done(null, false, { message: 'nao pode adicionar' }))
                ;
            })
            .catch(error => done(null, false, { message: 'nao pode achar' }))
          // if (user) {
